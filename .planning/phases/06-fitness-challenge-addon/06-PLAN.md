...
- Integrações fora do modulo sao minimas e explicitas:
  ...
  - `.env.example`: documenta `FEATURE_CHALLENGES_ADDON=false`.
  - `tests/Pest.php`: inclui os testes do modulo no bootstrap Laravel.
- As rotas sao registradas pelo service provider sob `/api/desafios`, com middleware `web`, `auth` e `challenges.enabled`.
...
- Upload de imagem/video usa o disk configuravel `CHALLENGES_MEDIA_DISK`; videos ainda nao geram thumbnail automatico porque isso exigiria FFmpeg/servico externo.
- As telas Volt ficam em `resources/views/livewire/desafios/`; rotas web usam nomes `desafios.app.*` para nao colidir com a API.
...
- `resources/views/livewire/desafios/*`
...
- `C:/xampp/php/php.exe artisan route:list --name=desafios`

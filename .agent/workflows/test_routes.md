---
description: Como testar todas as rotas da aplicação Laravel e gerar um relatório
---

Este workflow/SOP define o passo a passo para um agente testar automaticamente todos os endpoints (Rotas do tipo GET) de uma aplicação Laravel. Este fluxo simula requisições através de um usuário autenticado localmente com `role = 'admin'` durante o teste, capturando inclusive as mensagens de erro ou retorno (Ex: Exceptions) e salva o resultado final num formato Markdown.

// turbo-all

1. Escreva um script customizado no Artisan Command na rota de console do Laravel `app/Console/Commands/TestRoutes.php` com o seguinte conteúdo:
```php
<?php
...
2. Execute o novo comando adicionado na sua aplicação usando o wrapper do Sail:
```bash
./vendor/bin/sail artisan route:test
```

3. Exiba os resultados do arquivo salvo `route_test_report.md` no final para inspecionar possíveis exceptions e status codes.
```bash
cat route_test_report.md
```

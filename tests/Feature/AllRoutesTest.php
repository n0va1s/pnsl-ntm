    ...
    public function test_all_get_routes()
    {
        ...
        foreach ($routes as $route) {
            if (in_array('GET', $route->methods())) {
                ...
                if (strpos($uri, '_ignition') === 0) {
                    continue;
                }
                if (strpos($uri, 'sanctum') === 0) {
                    continue;
                }
                if (strpos($uri, 'livewire-e504497a') === 0) {
                    continue;
                }
                if (strpos($uri, 'livewire/update') === 0) {
                    continue;
                }
                if (strpos($uri, 'flux') === 0) {
                    continue;
                }
                if (strpos($uri, 'up') === 0) {
                    continue;
                }
                if (strpos($uri, '_debugbar') === 0) {
                    continue;
                }
                if (strpos($uri, 'storage') === 0) {
                    continue;
                }
                if (strpos($uri, 'limpar-tudo') === 0) {
                    continue;
                }
                if (strpos($uri, 'otimizar-tudo') === 0) {
                    continue;
                }
